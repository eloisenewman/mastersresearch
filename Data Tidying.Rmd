...
write_excel_csv(selectbehaviour, "behavioural_data_tidied.csv", col_names = TRUE)
```
...
2.  `pan_pal` - each row for each fly, including total ag_acts and presence of fencing.

    1.  `pan_pal_ag` - each row for each type of ag_act, so 6 rows for each fly.

3.  `selectbehaviour` - tidied full dataset with selected behaviours.

4.  `meta_data2` - tidied metadata

5.  `winners_losers` - has win, loss, draw for each fly separately
